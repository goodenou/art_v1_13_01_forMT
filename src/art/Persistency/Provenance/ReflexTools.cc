  ...
  typedef std::set<std::string> StringSet;
    ...
    StringSet & missingTypes() {
        ...
        if (s_missingTypes == 0) {
            ...
            if (s_missingTypes == 0) {
                s_missingTypes = new StringSet;
            }
      
        }
    return *s_missingTypes;
    }
  ...
  namespace
  {
    ...
    checkType(Type t, bool noComponents = false)
    {
      ...
      // Reflex maintains its own internal cache.
        ...
        StringSet* s_types = 0;
        std::mutex types_lock;
        
        if (s_types == 0) {
            std::lock_guard<std::mutex> guard(types_lock);
            
            if (s_types == 0) {
                s_types = new StringSet;
            }
